...
Curated visual styles for Beautiful Slides. Each preset is inspired by real design references—no generic "AI slop" aesthetics. **Abstract shapes only—no illustrations.**
...
For the full mandatory CSS block, see [REFERENCE.md](REFERENCE.md).
...
CSS does not allow a leading `-` before function names like `clamp()`, `min()`, `max()`. The browser silently discards the entire declaration. **Rule: Always use `calc(-1 * ...)` to negate CSS function values.**

---

## Troubleshooting Viewport Issues

See [REFERENCE.md](REFERENCE.md) for overflow prevention, density limits, and testing viewports.
